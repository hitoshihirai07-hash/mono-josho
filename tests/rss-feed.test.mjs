...
test("RSS 2.0としてPinterestに必要な画像と認証ドメインのリンクを出力する", () => {
  const xml = createRssXml({
    ...
    items: [{
      ...
      imageUrl: "https://mono-josho.pages.dev/brand/pinterest-games-weekly.png"
    }]
  });

  assert.match(xml, /<rss version="2\.0"/);
  assert.match(xml, /xmlns:media=/);
  assert.match(xml, /<media:content url="https:\/\/mono-josho\.pages\.dev\/brand\/pinterest-games-weekly\.png"/);
